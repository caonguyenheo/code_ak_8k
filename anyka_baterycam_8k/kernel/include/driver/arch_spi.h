...
/**
 * @brief master write and read  to spi port
 *
 * @author 
 * @date
 * @param[in] spi_id the spi port number, refer to T_eSPI_ID
 * @param[in] r_buf  the data buffer pointer
 * @param[in] w_buf the data buffer pointer
...
/**
 * @brief spi slave  write and read
 *
 * this func must be called in spi  slave mode
 * @author 
 * @date 2010-11-17
 * @param spi_id[in] spi interface selected
 * @param w_buf[in] the write data buffer  
 * @param r_buf[out] the read data buffer  
 * @param count[in] the  data count
 * @return bool
 * @retval true:   successfully.
 * @retval false:  failed.
 */
bool spi_dma_slave_write_read(T_eSPI_ID spi_id, unsigned char *w_buf, unsigned char *r_buf,unsigned long count);

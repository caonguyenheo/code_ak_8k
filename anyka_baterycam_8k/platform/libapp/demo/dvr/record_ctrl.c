 ...
 * return: void
 */
static void init_record_common(enum dvr_file_type file_type)
{	
    char file_name[128];
    T_SYSTIME systime;
...
static void check_plan_record_start(void)
{
    ...
    ak_dvr_record_start(RECORD_TRIGGER_TYPE_RESERVED);
...
static int check_record_exception(void)
{
    ...
    record_ctrl.except_type = DVR_EXCEPT_NONE ;
    return 0;
}
...
ak_pthread_t record_ctrl_init(void *vi_handle, void *ai_handle,
					enum dvr_file_type file_type , struct video_config * video_config)
{
	if(record_ctrl.run_flag){

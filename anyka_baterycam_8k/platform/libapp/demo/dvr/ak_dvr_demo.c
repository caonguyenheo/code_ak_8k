...
static int init_vi(void)
{
	...
	{
		//ak_print_normal("ymx:set fps:%d.\n",g_cap_fps);
    	ak_vi_set_fps(vi_handle, g_cap_fps);

    }
	int fps = ak_vi_get_fps(vi_handle);
	if(fps < g_video_config.savefilefps )
		g_video_config.savefilefps = fps;
...
*/    
	...
	ak_pthread_t dvr_thread_id = record_ctrl_init(vi_handle, ai_handle_global, DVR_FILE_TYPE_AVI,&g_video_config);

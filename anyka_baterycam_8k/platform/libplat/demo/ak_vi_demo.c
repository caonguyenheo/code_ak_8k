...
void cmd_vi_test(int argc, char **args)  
{
	...
	for (i = 0; i < argc; i++) {
		switch (i) {
			case 0:
				...
				char* p_end_tmp = NULL;
				framecount = strtol(args[i], &p_end_tmp, 10 );
				/* ymx : videmo  framecount no need greater than 10.*/
				if(framecount > 10 || *p_end_tmp != '\0')
				{
	...
	sprintf(filename, "a:/VI_%d%02d%02d_%02d%02d%02d_%s.YUV", systime.year, 

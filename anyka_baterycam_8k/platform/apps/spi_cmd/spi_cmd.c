...
int flip_update_bt = 0, flip_update_ud = 0, ck_bt = 0, ck_ud = 0;
	...
	*(pResp + SPIRESP_OFFSET_ID) = (*pCmd)&(~CC_SEND_SPI_PACKET_MASK);
	switch(cCmdType)
	{
		...
		case SPICMD_TYPE_SET:
			...
			{
				ak_print_normal_ex("flip_up_down value Old\r\n");
			}
			if(iRet != 0)
			{
				*(pResp + SPIRESP_OFFSET_DATA) = -1;
			...
			{
				ak_print_normal_ex("flip_bottom_top value Old\r\n");
			}
			if(iRet != 0)
			{
				*(pResp + SPIRESP_OFFSET_DATA + 1) = -1;

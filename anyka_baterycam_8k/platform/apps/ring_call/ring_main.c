...
static void send_picture_data(struct video_stream *p_stream)
{
    ...
    // send
      ...
      *(int*)opt_ptr = FRAME_TYPE_PICTURE;
      ...
      ak_print_normal("send picture type%d time %lu len %d\n", spi_send_buf.reserve,*(long*)&send_buf[8],frame_len);
      for(i =0;i < 16;i++)

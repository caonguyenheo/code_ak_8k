...
{
    g_pRingbuf = RingBuffer_create(2*1024*1024);   // 500KB
    if(g_pRingbuf == NULL)
    {
        return -1;
    }
...
/* set on flight */
extern int flip_update_bt, flip_update_ud, ck_ud, ck_bt;
...
void *ak_video_encode_task(void *arg)
{
    ...
    pucBufferH264 = (unsigned char *)malloc(CFG_MAX_FRAME_LEN *2);
    if(pucBufferH264 == NULL)
    {
        ak_print_error_ex("malloc pucBufferH264 failed!\n");
		goto exit_stream;
    }
    pucBufferSendSpi = (unsigned char *)malloc(CFG_MAX_FRAME_LEN *2);
...
// #endif
    ...
    while(g_video_start)
    {
        ...
        /* Tien Trung: debug time to send I Frame */
        // if(stream.frame_type == 1){
        //     ulCurrentTick = get_tick_count();
        //     ak_print_normal("Time to process IFrame %u\n", ulCurrentTick - ulLastTick);
        // }
       	ulCurrentTick = get_tick_count();
...
void *ak_video_take_snapshot_task_2(void *arg)
{
    ...
    while(1)
    {
        ...
        if(g_iP2pSdStreamStart == 0) // p2p sdcard is streaming
        {
            ...
            while(1)
            {
                iRet = video_take_snapshot_one_frame(g_camera_handle, jpeg_handle, &stream, 3);
                if (0 != iRet)
                {
                    ak_print_error_ex("encode JPEG error!\n");
                }

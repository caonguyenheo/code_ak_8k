...
static int talkback_decoder_init(void)
{
    int s32Ret = 0;
#ifdef CFG_TALKBACK_G722
    s32Ret = g722_dec_init(&g_dec_g722);
#endif
    return s32Ret;
}
...
static int talkback_decode_data(char *i8DataIn, int i32LenIn, char *i8DataOut, int *i32LenOut)
{
    ...
    {
        u32NSample = 2*i32LenIn;
#ifdef CFG_TALKBACK_G722
        //printf("[G722] u32NSample: %d\n\r", u32NSample);
        g722_dec_decode(&g_dec_g722, i8DataIn, (size_t)i32LenIn, \
                                        (int16_t *)i8DataDecode, &u32NSample);
...
static int ao_send_frame_maintask(void * ao_handle)
{
    ...
    else if(g_voicevalue == SPICMD_TYPE_VOICE_JINGLE)
    {
    	...
    	ret = aktb_play_data_over_spk(ao_handle,Ding_Data,len_arrding);
    	g_receive_update = 0;
    ...
    else if(g_voicevalue == SPICMD_TYPE_EN_VOICE_SUCCESS)
    {
        ...
        ret = aktb_play_data_over_spk(ao_handle,Success_Data,len_arrsuccess);
        g_receive_update = 0;
    ...
    else if(g_voicevalue == SPICMD_TYPE_EN_VOICE_FAIL)
    {
        ...
        ret = aktb_play_data_over_spk(ao_handle,Fail_Data,len_arrfail);
        g_receive_update = 0;
    ...
    else if(g_voicevalue == SPICMD_TYPE_EN_VOICE_TIMEOUT)
    {
        ...
        ret = aktb_play_data_over_spk(ao_handle,Timeout_Data,len_arrtime);
        g_receive_update = 0;
    ...
    else if(g_voicevalue == SPICMD_TYPE_EN_VOICE_CONNECTED)
    {
    	ak_print_normal("LEN_EN_CONNECTED: %d\r\n",LEN_EN_CONNECTED);
    	ret = aktb_play_data_over_spk(ao_handle,EN_CONNECTED,LEN_EN_CONNECTED);
    	g_receive_update = 0;
    	ak_print_normal("EN CONNECTED PLAY DONE\r\n");
    }
    ...
    {
    	ak_print_normal("LEN_CH_CONNECTED: %d\r\n",LEN_EN_CONNECTED);
    	ret = aktb_play_data_over_spk(ao_handle,EN_CONNECTED,LEN_EN_CONNECTED);
    	g_receive_update = 0;
    ...
    {
    	ak_print_normal("LEN_CH_UNABLETOCONNECT: %d\r\n",LEN_EN_UNABLETOCONNECT);
    	ret = aktb_play_data_over_spk(ao_handle,EN_UNABLETOCONNECT,LEN_EN_UNABLETOCONNECT);
    	g_receive_update = 0;
    ...
    {
    	ak_print_normal("LEN_CH_WARNINGWIFIISWEAK: %d\r\n",LEN_EN_WARNINGWIFIISWEAK);
    	ret = aktb_play_data_over_spk(ao_handle,EN_WARNINGWIFIISWEAK,LEN_EN_WARNINGWIFIISWEAK);
    	g_receive_update = 0;
    ...
    while(1)
    {
        ...
        else
        {
            ...
            if(i32LenRb >= (LEN_DATA_PLAY) )
            {
                ...
                {
                    i32Ret = aktb_play_data_over_spk(ao_handle, (char *)pBuffPlay, i32SizeTalkback);
